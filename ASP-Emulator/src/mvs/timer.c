...
void (*timer_update_cpu)(int);
static void timer_update_cpu_normal(int);
static void timer_update_cpu_raster(int);
...
static void timer_update_cpu_normal(int skipframe)
{
	int i, time;
...
static void timer_update_cpu_raster(int skipframe)
{
	int i, time;
	...
	for (scanline = 1; scanline <= RASTER_LINES; scanline++)
	{
		...
		neogeo_raster_interrupt(scanline,skipframe);
	}
